  ...
  useEffect(() => {
    const fetchProductsAndUser = async () => {
      try {
        ...
        if (prodData.success) {
          let farsanProducts = prodData.data.filter(
            (product: IProductResponse) =>
              product.parentCategory?.name?.toLowerCase() === "farsan"
          );
          ...
          if (userData.success && userData.data) {
            ...
            farsanProducts = farsanProducts.map(
              (product: IProductResponse) => ({
                ...product,
          ...
          // Set up product data
          setProducts(farsanProducts);

          const primaryCatMap = new Map<
            string,
            { id: string; name: string; count: number }
          >();
          farsanProducts.forEach((product: IProductResponse) => {
          ...
          // Price Range
          const prices = farsanProducts.map(
