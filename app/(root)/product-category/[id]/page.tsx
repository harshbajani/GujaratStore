  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        if (prodData.success) {
          let filteredProducts = prodData.data.filter(
          ...
          >();
          filteredProducts.forEach((product: IProductResponse) => {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="dynamic-container mx-auto px-4">
        <motion.h1
          ...
          className="text-3xl font-bold mb-8 capitalize py-10"
        >
          Search results for: {products[0]?.primaryCategory?.name || "Products"}
        </motion.h1>
            ...
            >
              ...
              {(filters.secondaryCategories.length > 0 ||
                ...
                <span className="ml-1 rounded-full bg-brand px-2 py-0.5 text-xs text-white">
                  {filters.secondaryCategories.length + filters.colors.length}
                </span>
              )}
            </Button>
          </div>
                            ...
                            <div className="mb-2 flex items-center gap-2">
                              ...
                              {product.mrp > product.netPrice && (
                                <span className="text-sm text-gray-500 line-through">
                                  ₹
                                  {Math.floor(product.mrp).toLocaleString(
                                    "en-IN"
                                  )}
                                </span>
                              )}
