...
import { adminBrandSchema } from "../../validations";
...
type BrandFormData = z.infer<typeof adminBrandSchema>;

export async function createBrand(data: BrandFormData) {
  try {
    await connectToDB();
    const validatedData = adminBrandSchema.parse(data);
...
export async function getBrandById(id: string): Promise<IAdminBrand | null> {
  try {
    await connectToDB();
    ...
    const brand = (await Brand.findById(id)) as IAdminBrand | null;
